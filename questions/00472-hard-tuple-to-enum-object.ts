...
type PascalCase<T extends string> = `${T}` extends `${infer F}${infer R}`
  ? `${Uppercase<F>}${R}`
  : T

type Enum<T extends readonly string[], N extends boolean = false, U extends any[] = []> = 
T['length'] extends U['length']
  ? {}
  : {
    readonly [K in (keyof Enum<T, N, [...U, 1]> | PascalCase<T[U['length']]>)]: 
      K extends keyof Enum<T, N, [...U, 1]> 
      ? Enum<T, N, [...U, 1]>[K] 
      : N extends false 
        ? T[U['length']]
        : U['length']
  }
type t1 = Enum<typeof OperatingSystem>
