...
P = O,
D extends any[] = []> = {
  [K in keyof O]: O[K] extends object ? DeepObjectToUniq<O[K], P, [...D, K]> : O[K];
} & {
  [KEY]?: [P,D]
}

// type DeepObjectToUniq<O extends object> = O
...
type t1 = keyof Foo & string
type t2 = keyof UniqFoo & string
